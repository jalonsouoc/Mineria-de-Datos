...
Para realizar laas agrupaciones utilizaremos sólo los datos numéricos en este caso serán las variables Wing, Weight, Culmen, Halloux. Tail, StandardTail, Tarsus, WingPitFat, KeelFat y Crop.
...
Realizamos unas últimas comprobaciones ahora empleando la función kmeansruns en función a los cirterios de la silueta media y Calinski-Harabasz
````{r, message=FALSE, Warning=FALSE}
...
Vemos que con el cirterio de la silueta de la media obtenemos dos clústers y con el criterio de Calinski-harabas obtenes 10 clústers.


El caso de estudio sabemos que se pueden agrupar los datos en 3 especies de halcones (ratonero de cola roja, gavilán americano, gavilán de cooper) comprobaremos el comportamiento del algoritmo kmeans para 3 clústers cogiendo de dos en dos las variables.
...
Como podemos observar ninguna de las variables parece haber acertado en la clasificación en comparación con la clasificación real debido a que nos encontramos que la clasificación real es bastante dispersa. Por lo que no aprecen datos relevantes a la hora de clasificar el tipo de halcón de entre las especies de ratonero de cola roja, gavilán americano, gavilán de cooper
...
Analizaremos ahora la muestra de datos de los halcones empleando los algoritmos de DBSCAN y OPTICS. Al igual que en el apartado anterior, aplicaremos el algoritmo a las variables de dos en dos t con una densidada mínima ceptada de 10.
...
````{r, message=FALSE, warning=FALSE}
plot(HawksTrait[c(3,4)],col="grey")
polygon(HawksTrait[resOptics34$order,])
head(HawksTrait[resOptics34$order,])
````
...
````{r, message=FALSE, warning=FALSE}
plot(HawksTrait[c(1,5)],col="grey")
polygon(HawksTrait[resOptics15$order,])
````


````{r, message=FALSE, warning=FALSE}
head(HawksTrait[resOptics15$order,])
````
