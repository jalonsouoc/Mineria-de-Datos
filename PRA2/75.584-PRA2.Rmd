...
En este caso vamos a emplear un algoritmo de agrupamiento jerárquico, para ello emplearemos la función en R hclust. Como métrica para obtner la distancia Manhattan (geometría del taxi). Comprobamos el comportamiento para 4 clústeres.
...
Observamos que nuevamente tenemos  un gran clúster que alberga la mayor parte de la muestra, el cual podría referenciar a los clústeres de la zona central obtenidos en apartados anterirores, en este caso vemos que se forman 2 clústeres con bastantes datos en comparación al algoritmo anterior y un último clúster con una muestra ínfima de datos lo que pdoría corresponder  las zonas exteriores.
...
Nuevamente se diferencia un gran clúster en relación con el resto de clústeres de menor tamaño, nuevamente podríamos considerar que es la zona central de madrid y el resto de clústeres más pequeños podrían hacer refrencia a las zonas de las afueras de la comunidad.
...
Vemos que este modelo para la muestra de testeo acierta en un 69.92 % de los casos comprobamso que es un porcentaje inferior al obtenido en el apartado anterior.
...
Podemos ver que este algoritmo tiene un porcentaje de acierto menor que el obtenido en el ejercicio aterior, también observamos que de todas las variables sólo considera relevante la variable tipo_vehiculo, es cierto que el modelo anteriro también la tenía como la más relevante pero tomaba en cuenta algunas de las otras variables. Por último destacar la incapacidad del modelo de predecir los accidentes Graves y Mortales.
